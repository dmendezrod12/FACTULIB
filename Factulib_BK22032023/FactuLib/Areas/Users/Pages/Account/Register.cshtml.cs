            ...
            if (dataUser == null)
            {
                ...
                if (_dataUserActualizar2 == null && id != 1 && id != 2)
                {
                    if (User.IsInRole("Administrador"))
                    {
                        ...
                        {
                            return Redirect("/Users/Register?id=3");
                        }
                    }
                    else
                    {
                    ...
                    await strategy.ExecuteAsync(async () =>
                    {
                        using (var transaction = _context.Database.BeginTransaction())
                        {
                            try
                            {
                                ...
                                {
                                    _dataInput.ErrorMessage = "CONTRASEÑA NO CUMPLE CON LOS REQUISITOS: ";
                                    foreach (var item in result.Errors)
                                    {
                                        if (item.Description == "Passwords must have at least one non alphanumeric character.")
                                        {
                                            _dataInput.ErrorMessage = _dataInput.ErrorMessage + "Debe contener al menos 1 simbolo no alfa numérico ('$','!','#'...) ,";
                                        }
                                        if (item.Description == "Passwords must have at least one lowercase ('a'-'z').")
                                        {
                                            _dataInput.ErrorMessage = _dataInput.ErrorMessage + "Debe contener al menos una minúscula ('a'-'z'),";
                                        }
                                        if (item.Description == "Passwords must have at least one uppercase ('A'-'Z').")
                                        {
                                            _dataInput.ErrorMessage = _dataInput.ErrorMessage + "Debe contener al menos una mayúscula ('A'-'Z'),";
                                        }
                                        if (item.Description == "Passwords must have at least one digit ('0'-'9').")
                                        {
                                            _dataInput.ErrorMessage = _dataInput.ErrorMessage + "Debe contener al menos un número ('0'-'9'),";
                                        }
                                    }
                                    valor = false;
                                }
