...
namespace FactuLib.Library
{
    public class LCompras : ListObject
    {
        ...
        public DataPaginador<TProveedor> GetProveedoreslista(int page, int num, String valor, HttpRequest request)
        {
            ...
            {
                data.Reverse();
                objects = new LPaginador<TProveedor>().paginador(data, page, num, "Compras", "Compras", "ListaProveedores", url);
            }
            else
            {
                objects[0] = "No hay datos que mostrar";
        ...
        public DataPaginador<TProducto> GetProductosLista(int page, int num, String valor, HttpRequest request)
        {
            ...
            {
                data.Reverse();
                objects = new LPaginador<TProducto>().paginador(data, page, num, "Compras", "Compras", "ListaProductos", url);
            }
            else
            {
                objects[0] = "No hay datos que mostrar";
        ...
        public List<TProducto> GetProductos(String valor)
        {
            ...
            if (valor == null)
            {
                listTemporal = _context.TProducto.Where(c=> c.Enabled.Equals(true)).ToList();
            }
            else
            {
                listTemporal = _context.TProducto.Where(c => (c.Nombre_Producto.StartsWith(valor) || c.Codigo_Producto.Equals(valor)) & c.Enabled.Equals(true)).ToList();
            }
            if (!listTemporal.Count.Equals(0))
            {
                foreach (var item in listTemporal)
